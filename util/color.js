...
var XYPoint = function(x, y, bri) {
		this.x = x;
		this.y = y;
    this.bri = bri;
	};

const ModelA =  ['LST001', 'LLC010', 'LLC011', 'LLC012', 'LLC006', 'LLC005', 'LLC007', 'LLC014', 'LLC013'];
const ModelB = ['LCT001', 'LCT007', 'LCT002', 'LCT003', 'LLM001'];
const ModelC = ['LCT010', 'LCT014', 'LCT011', 'LLC020', 'LST002', 'LCT012'];

export const rgbToCIE1931 = function(red, green, blue, modelId) {
	var point = _getXYPointFromRGB(red, green, blue, modelId);
	return [point.x, point.y, point.bri];
};
...
function _getXYPointFromRGB(red, green, blue, modelId) {
  if (ModelA.includes(modelId)) {
    Red = new XYPoint(0.704, 0.296);
    Lime = new XYPoint(0.2151, 0.7106);
    Blue = new XYPoint(0.138, 0.08);
	...
	var xyPoint = new XYPoint(cx, cy),
		inReachOfLamps = _checkPointInLampsReach(xyPoint, modelId);
	if (!inReachOfLamps) {
	...
	return new XYPoint(cx, cy, Y);
}

export const getRGBFromXYAndBrightness = function (x, y, bri, modelId) {
